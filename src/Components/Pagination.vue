...
<template>
  <div v-if="props.query" class="flex flex-col gap-4 bg-[#1a1a1a] p-4 rounded-t-lg border-x border-t border-gray-800">
    ...
    <div class="flex flex-wrap items-center gap-3">
      ...
      <div
        class="flex items-center flex-grow max-w-md bg-[#252525] border border-gray-700 rounded-md px-3 py-1.5 focus-within:border-blue-500 transition-all">
        ...
        <input v-model="searchQuery" type="text"
          ...
          placeholder="Buscar pelo título..." @keyup.enter="onSearch" />
      </div>

      <div class="flex bg-[#252525] rounded-md border border-gray-700 p-1">
        <button @click="showSteam()"
          class="px-3 py-1 rounded text-sm text-gray-400 hover:bg-gray-700 hover:text-white transition-colors">
          <i class="bi bi-steam"></i>
        </button>
        <button @click="showPsn()"
          class="px-3 py-1 rounded text-sm text-gray-400 hover:bg-gray-700 hover:text-white transition-colors border-l border-gray-700">
          <i class="bi bi-playstation"></i>
        </button>
        <button @click="showSwitch()"
          class="px-3 py-1 rounded text-sm text-gray-400 hover:bg-gray-700 hover:text-white transition-colors border-l border-gray-700">
      ...
      <div class="flex">
        ...
        <select v-model="query.midia"
          class="bg-[#252525] text-sm text-gray-300 border border-gray-700 rounded-md px-3 py-1.5 outline-none focus:border-gray-500">
      ...
      <div class="flex">
        ...
        <select v-model="query.statusCompra"
          class="bg-[#252525] text-sm text-gray-300 border border-gray-700 rounded-md px-3 py-1.5 outline-none focus:border-gray-500">
          <option value="" selected="true">Status Compra</option>
          <!-- Obtido do array de opções -->
          <template v-for="option in optionsStatusCompra">
      ...
      <div>
        ...
        <select v-model="query.status"
          ...
          <!-- Obtido do array de opções -->
          <template v-for="option in optionsStatus">
            <option :value="option.key">{{ option.value }}</option>
          </template>
        </select>
      </div>

      <button @click="onSearch" class="flex bg-[#252525] rounded-md border border-gray-700 px-2.5 py-1.5"><i
          class="bi bi-search"></i></button>
    </div>

    <div class="flex lg:flex-row items-center lg:justify-between border-t border-gray-800 gap-4 py-2">
      <div class="flex items-center gap-2">
        ...
        <span class="text-sm text-gray-500">
          ...
          -
          <span class="text-gray-300">{{ Math.min((props.query.page + 1) * props.query.limit, props.query.totalCount)
          }}</span>
          de
